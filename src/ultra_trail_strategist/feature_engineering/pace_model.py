...
class PacePredictor:
    ...
    def prepare_training_data(self, activities_streams: List[Dict[str, Any]]) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def predict(self, grade: float) -> float:
        ...
        if not self.is_fitted:
            ...
            # Very rough fallback
            return max(0.5, 2.78 - (abs(grade) * 0.1)) 
            
        return self.model.predict([[grade]])[0]
