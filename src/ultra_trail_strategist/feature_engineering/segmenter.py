...
class CourseSegmenter:
    ...
    def _classify_points(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        type_expr = (
            pl.when(pl.col("grade") > self.climb_threshold).then(pl.lit(SegmentType.CLIMB.value))
            .when(pl.col("grade") < self.descent_threshold).then(pl.lit(SegmentType.DESCENT.value))
    ...
    def _create_segments(self, df: pl.DataFrame) -> List[Segment]:
        ...
        df = df.with_columns(
            (pl.col("segment_type") != pl.col("segment_type").shift(1).fill_null(pl.col("segment_type").first()))
            .cum_sum()
            .alias("group_id")
        )
